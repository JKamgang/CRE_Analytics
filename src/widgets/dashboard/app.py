...
from src.shared.i18n.config import I18nConfig
from src.widgets.excel_export.generator import ExcelProGenerator
from src.widgets.powerbi_export.generator import PowerBIGenerator
from src.widgets.colab_generator.generator import ColabGenerator
...
with st.sidebar:
    ...
    data_source = st.radio(
        ...
        disabled=not st.session_state.tier.has_real_time_api()
    )
    use_live = data_source == "Live API + Sample Fallback" and st.session_state.tier.has_real_time_api()
...
# ──────────────────────────────────────────────────────────────────────────
# TAB 6: EXPORTS & SCAFFOLDS
# ──────────────────────────────────────────────────────────────────────────
with tab_exports:
    st.subheader("💾 Multi-Option Export & Scaffolding")
    st.markdown("Generate Pro Excel files, Power BI scaffolds, and Colab notebooks.")
    ...
    with col1:
        if st.button("Generate Pro Excel File"):
            if not df.empty:
                ...
                st.success(f"Generated Excel: {out}")
                with open(out, 'rb') as f:
                    st.download_button("⬇️ Download Excel", f, file_name="Alile_CRE_Growth_Pro.xlsx")
            else:
                st.warning("No data to export.")

    with col2:
        if st.button("Generate Power BI Scaffold"):
            gen = PowerBIGenerator()
            out = gen.generate_instruction_file()
            st.success(f"Generated Power BI instructions: {out}")
            with open(out, 'rb') as f:
                st.download_button("⬇️ Download Power BI Scaffold", f, file_name="PowerBI_Instructions.txt")

    with col3:
        if st.button("Generate Colab Notebook"):
            gen = ColabGenerator()
            out = gen.generate_notebook()
            st.success(f"Generated Colab Notebook: {out}")
            with open(out, 'rb') as f:
                st.download_button("⬇️ Download Colab Notebook", f, file_name="Alile_CRE_Free_Playground.ipynb")
...
with tab_learning:
    ...
    st.markdown("**AI Insight Request:**")
    st.info(prompt)

    if st.button("Generate Insight"):
        with st.spinner("Routing request through LLM Cascade..."):
            response = llm_router.route_request(prompt)
...
st.divider()
st.markdown(
    "<center><small>Alile CRE Analytics v2.0 · Built for Project REAP · "
