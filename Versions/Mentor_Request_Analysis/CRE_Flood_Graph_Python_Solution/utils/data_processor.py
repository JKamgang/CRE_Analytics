...
from config import DC_CONFIG, ATLANTA_CONFIG, PROCESSED_DATA_DIR, RAW_DATA_DIR, SECTORS
...
def load_or_fetch(city: str, force_refresh: bool = False) -> pd.DataFrame:
    ...
    if city.upper() in ("DC", "WASHINGTON"):
        cfg = DC_CONFIG
        from pipelines.dc_pipeline import run_dc_pipeline as pipeline_fn
    elif city.upper() in ("ATL", "ATLANTA"):
        cfg = ATLANTA_CONFIG
        from pipelines.atlanta_pipeline import run_atlanta_pipeline as pipeline_fn
    else:
